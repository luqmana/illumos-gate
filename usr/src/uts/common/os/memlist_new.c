	...
	uint64_t flags)
{
	...
	/*
	 * Inserting a zero-length span is no-op with relaxed semantics.
	 */
    ...
    uint64_t bytes, struct memlist **curmemlistp)
{
	...
	/*
	 * Deleting a zero-length span is a no-op under relaxed semantics.
	 */
	if (bytes == 0)
		return (MEML_SPANOP_OK);
    ...
    uint64_t *addrp)
{
	...
	for (; ml != NULL; ml = ml->ml_next) {
		...
		if (delta != 0) {
			if (ml->ml_size >= size + align - delta)
				break;
		} else if (ml->ml_size >= size) {
			break;
		}
	}
    ...
    uint64_t address, uint64_t size, uint64_t align, uint64_t *addrp)
{
	...
	delta = (align == 0) ? 0 : (ml->ml_address & (align - 1));
	if (ml->ml_size < ((delta != 0) ? size + align - delta : size))
		return (MEML_SPANOP_ESPAN);
...
int
memlist_rsrc_add(uint64_t address, uint64_t bytes, struct memlist **listp)
{
	return (xmemlist_add_span(&memlist_kmem_pool, address, bytes, listp,
	    MEML_FL_RELAXED));
}

int
memlist_rsrc_delete(uint64_t address, uint64_t bytes, struct memlist **listp)
{
	return (xmemlist_delete_span(&memlist_kmem_pool, address, bytes, listp,
	    MEML_FL_RELAXED));
}
...
{
	return (xmemlist_delete_list(&memlist_kmem_pool, listp, del,
	    MEML_FL_RELAXED));
}
...
{
	return (xmemlist_merge(&memlist_kmem_pool, src, destp,
	    MEML_FL_RELAXED));
}
...
{
	return (xmemlist_subsume(&memlist_kmem_pool, srcp, destp,
	    MEML_FL_RELAXED));
}
